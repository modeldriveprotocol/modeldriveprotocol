...
function getSidebarGroups() {
  return Array.from(
    document.querySelectorAll<HTMLElement>('#VPSidebarNav .group')
  )
}
...
function getChildItems(item: HTMLElement) {
  ...
  return Array.from(container.children).filter(
    child =>
      child instanceof HTMLElement && child.classList.contains('VPSidebarItem')
  ) as HTMLElement[]
}
...
<template>
  <div class="mdp-sidebar-filter">
    <label class="visually-hidden" for="mdp-sidebar-filter-input">{{
      copy.label
    }}</label>
    <div class="mdp-sidebar-filter__field">
      <span class="mdp-sidebar-filter__icon" aria-hidden="true" />
      <input
    ...
    <p v-if="hasNoResults" class="mdp-sidebar-filter__empty">
      {{ copy.noResults }}
    </p>
  </div>
</template>
