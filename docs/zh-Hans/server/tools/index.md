...
server 仍然保留 `listTools`、`listPrompts`、`listSkills`、`listResources`、`callTools`、`getPrompt`、`callSkills`、`readResource`、`callClients` 这些 legacy alias，方便迁移。
它们本质上只是 canonical path model 之上的兼容层，因此这里不再为它们保留一组一组的独立文档页。
