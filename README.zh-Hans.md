...
path model 是当前主 API。为了迁移，JavaScript client 仍然保留 `exposeTool` / `exposePrompt` / `exposeSkill` / `exposeResource` 这些 legacy 包装层，MCP bridge 也仍然提供旧的 `listTools` / `callTools` / `getPrompt` / `readResource` 这类兼容别名。
