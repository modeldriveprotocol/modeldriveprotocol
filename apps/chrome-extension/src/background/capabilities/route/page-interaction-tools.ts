...
export function registerPageInteractionTools(
  ...
  for (const [toolName, path, commandType, description, required] of [
    ['page.queryElements', '/page/query-elements', 'queryElements', 'Query DOM elements from the target page.', ['selector']],
    ['page.click', '/page/click', 'click', 'Click a DOM element on the target page.', ['selector']],
    ['page.fill', '/page/fill', 'fill', 'Fill an input, textarea, select, or contenteditable element.', ['selector', 'value']],
    ['page.focus', '/page/focus', 'focus', 'Move focus to a matching element on the target page.', ['selector']]
  ] as const) {
    client.expose(
      path,
      {
